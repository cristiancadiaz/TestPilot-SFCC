...
- [ ] U4 API — integrates all; lands TASK-005 translator/api dedup + IaC CDK + DynamoDB store + `save_run` orchestration. **Recommended next.** Demo milestone: CLI smoke demo now possible (U1+U2+U3, no API/dashboard) → full E2E demo = wave-1 DoD after U4.
