"""Flow: checkout_full — full checkout with payment failure validation.

10-step flow that exercises the complete purchase journey:
  env_access_auth → shopper_login → search_product → category_page →
  pdp_variant_select → add_to_cart → mini_cart_validation →
  checkout_shipping → checkout_payment → payment_failure_validation

Invariant #1 (zero contamination): payment ALWAYS fails at step 10.
``orders_created`` is ALWAYS 0 — asserted before returning.
...
async def _step_env_access_auth(
    ...
    try:
        # Navigate to the storefront. Playwright handles HTTP basic auth via
        # browser context credentials configured in the runner (ADR-001).
...
async def _step_checkout_payment(page: "Page") -> StepResult:
    """Step 9 — fill payment form with the test card (always-fail card)."""
    name = "checkout_payment"
    start = time.monotonic()
    try:
        await page.wait_for_selector(SFCCSelectors.PAYMENT_CARD_NUMBER)
        await page.fill(SFCCSelectors.PAYMENT_CARD_NUMBER, SFCCSelectors.TEST_CARD_NUMBER)
        await page.fill(SFCCSelectors.PAYMENT_EXPIRY, SFCCSelectors.TEST_CARD_EXPIRY)
        await page.fill(SFCCSelectors.PAYMENT_CVV, SFCCSelectors.TEST_CARD_CVV)
        await page.click(SFCCSelectors.PAYMENT_SUBMIT)
        duration_ms = int((time.monotonic() - start) * 1000)
        return StepResult(name=name, status="success", duration_ms=duration_ms)
    except Exception as exc:  # noqa: BLE001
        duration_ms = int((time.monotonic() - start) * 1000)
        logger.warning("checkout_payment failed: %s", type(exc).__name__)
        return StepResult(
            name=name,
            status="failed",
            duration_ms=duration_ms,
...
async def _step_payment_failure_validation(
    page: "Page",
    run_id: str,
    profile_id: str,
) -> StepResult:
    """Step 10 — verify payment failed (invariant #1: always fails).

    ``status="success"`` means the expected failure was confirmed.
    This is the FINAL step — screenshot is always captured (ADR-003).
    """
    name = "payment_failure_validation"
    start = time.monotonic()
    screenshot_path = _build_screenshot_path(run_id, profile_id, name, "final")
    try:
        await page.wait_for_selector(SFCCSelectors.PAYMENT_ERROR_MESSAGE)
        duration_ms = int((time.monotonic() - start) * 1000)
        return StepResult(
            name=name,
            status="success",
        ...
        duration_ms = int((time.monotonic() - start) * 1000)
        logger.warning("payment_failure_validation failed: %s", type(exc).__name__)
...
async def run(
    ...
    """Execute the checkout_full flow (10 steps).
    ...
    for step_name in step_names:
        ...
        if step_name == "env_access_auth":
            step_result = await _step_env_access_auth(page, env, run_id, profile_id)
        elif step_name == "shopper_login":
            step_result = await _step_shopper_login(page, env)
        elif step_name == "search_product":
            step_result = await _step_search_product(page, config, run_id, profile_id)
            ...
            step_result = await _step_checkout_payment(page)
        elif step_name == "payment_failure_validation":
            step_result = await _step_payment_failure_validation(page, run_id, profile_id)
        else:
            # Should never be reached — catalog is closed.
        ...
        if step_result.status == "failed":
            failed = True
            # Overwrite screenshot on a non-final failed step (ADR-003).
            if step_name != "payment_failure_validation":
                if step_result.screenshot_state != "fail":
                    # Patch screenshot_state to "fail" if not already set.
                    steps[-1] = step_result.model_copy(
                        update={"screenshot_state": "fail"}
                    )

    # Mark the last executed step as "final" if it succeeded (ADR-003).
    ...
    if last_executed is not None and last_executed.status == "success":
        ...
        # Only patch if not already tagged (payment_failure_validation tags itself).
