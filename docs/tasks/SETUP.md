...
Requiere: workspace Linear + token + `project-slug` (+ `team-key` si hay varios teams), y los skills reales de OpenSymphony en `.agents/skills/`.
...
uv run --script .agents/skills/convert-tasks-to-linear/scripts/convert_tasks_to_linear.py dry-run   --manifest docs/tasks/task-package.yaml
# Publicar (issues en estado Todo)
uv run --script .agents/skills/convert-tasks-to-linear/scripts/convert_tasks_to_linear.py apply --manifest docs/tasks/task-package.yaml --project-slug [project-slug]
```
...
- `.agents/skills/custom-codereview-guide.md` — guía de review.
